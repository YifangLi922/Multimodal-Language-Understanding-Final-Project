...
- **为什么不算（踩中 exclusion）**：画面是一只刚刚破茧而出的蝴蝶，下方能看到已经空了的茧壳。茧壳虽然暗示了"之前有东西在里面"，但图片并没有真正拍到蛹/幼虫本身的样子，实际呈现的只是"蝴蝶已经出来了"这一个终结状态——本质上仍然是**单一阶段的定格瞬间**，达不到 inclusion 里"能看到 ≥2 个状态"的要求。踩中的是"caption 提到 transform/progress/stages 等词，但图片实际只呈现了单一的最终状态"这条 exclusion。这一对（正例 1 vs 边界反例 1）是很好的教学案例：正例 1 明确呈现了多个发育阶段，边界反例 1 虽然有个空茧壳作为"曾经变化过"的痕迹，但画面定格的仍然只是变化完成后的单一瞬间，不构成"多阶段"。
