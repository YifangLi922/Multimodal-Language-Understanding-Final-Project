...
- **为什么不算（踩中 exclusion）**：跟正例 1 是同一个主题（蝴蝶变态发育），但这张图的链接指向的是一幅**单只成年蝴蝶的艺术印刷画**，画面里只有一个静止的最终形态，看不到蛹或幼虫阶段——典型的"caption 声称有 transformation，但图片只呈现单一最终状态"。这一对（正例 1 vs 边界反例 1）是很好的教学案例：同一个话题，一张真的展示了多阶段，另一张只是 caption 用词像，图片内容对不上。
...
- **为什么不算（踩中 exclusion）**：一个迷你购物车摆在一堆金币上的静态摆拍，用来**象征**"进步"这个抽象概念，但画面本身是单一场景、没有真实的状态变化过程——踩中"静态象征/比喻画面"这条排除标准。
